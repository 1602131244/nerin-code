...
var specialty = [];
var dlvrs = '';
var nbccTaskName = null;
var chapterId = null;
var taskHeaderId = null;
var inDlvrId = null;
var requestid = null;
var erpid = null;
var isAppr = null;
	...
	$.ajax({
		...
		success: function(logondata) {
			if(!logondata.userId || logondata.userId == 0){
				...
				//可能完全由查询dlvrId或requestid获取prjid、prjPhaseID
				prjid = getUrlParam("projID");  //url必须载入的参数
				prjPhaseID = getUrlParam("phaseID");//url必须载入的参数
				//chapterId = getUrlParam("chapterId");
				taskHeaderId = getUrlParam("taskHeaderId");
				inDlvrId = getUrlParam("inDlvrId");
				requestid = getUrlParam("requestid");
				erpid = getUrlParam("erpid");
				isAppr = getUrlParam("isAppr");
				...
				$.ajax({
					...
					success: function (data) {
						...
						if(wbsStat=="F"){
							...
							jQuery.getScript("js/chr_center.js");
						}
					},
					error:function(data){
						$('#loaddiv').window('close');
