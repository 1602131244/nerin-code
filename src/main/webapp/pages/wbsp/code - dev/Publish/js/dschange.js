...
var specialty = [];
var phaseCode;
var isHighphase = false;
var HighphaseArr = ["BD","FS","PC","SD","UD"];
var phaseDraws;
var plancnDrawingNum = '';
var plandrawingNum = '';
var pmlist;
var pmNum;
$(function () {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if(wbsStat=="F"){
								...
								onLoadSuccess: function (data) {
									...
									$.getJSON("../../common/phaseList.json",function(jsondata){
										for(var i=0;i<jsondata.length;i++){
											if(jsondata[i].phaseID==prjPhaseID){
												phaseCode = jsondata[i].phaseCode;
												var phaseHead = phaseCode.substr(0,2);
												if($.inArray(phaseHead,HighphaseArr)>-1) isHighphase = true;
												break;
											}
										}
									});
							...
							$.ajax({
								url: "http://"+window.location.host+"/api/naviWbpsRest/drawingNum?projectId="+prjid,
								type: "GET",
								success: function (data) {
									phaseDraws = data;
									for(var i=0;i<phaseDraws.length;i++){
										if(phaseDraws[i].phaseId==prjPhaseID){
											plancnDrawingNum = phaseDraws[i].cnDrawingNum;
											plandrawingNum = phaseDraws[i].drawingNum;
											break;
										}
									}
