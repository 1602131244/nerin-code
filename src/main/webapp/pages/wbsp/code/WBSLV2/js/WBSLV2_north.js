...
function SwitchProjectPhase(prjid, phaseID) {
	if (($(subitemNoEdting).is(':visible') && !$('#subitem_toolbar_save').linkbutton('options').disabled) || $(subitemEdting).is(':visible')) {
	...
	}else window.location.href = "http://"+window.location.host+window.location.pathname+"?projID="+prjid+"&phaseID="+phaseID;
}